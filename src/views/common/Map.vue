...
/***
 *  echarts注册地图有两种方法
 * 1. series里面配置type,map来进行地图的展示
 *    map想要修改地图的背景颜色,是需要手动设置data中的起始值,并且在serires的同级别设置dataRange
 * 2. 使用坐标系geo来进行地图的展示
 *    通过regions设置不同的区域展示背景颜色
 *
 * 注册地图有两种方法
 * 1. 直接调用echarts的方法registerMap("命名", json)
 * 2. 引入一个js文件,在js文件中会完成地图的注册(对v3不太友好,因为他更多针对的v2)
 *
 *
 * 作业: 1. 没有吧地图展示出来的展示出来
 *      2. 两种方法设置地图的背景颜色,并且去官网查看两种方法都有哪些配置属性
 */
...
onMounted(() => {
  ...
  let myChart = echarts.init(chinaDom as any)
